...
This repository contains supplementary material accompanying the article entitled '**Modelling wildlife habitat selection from tracking data: A practical guide for ecologists**', submitted to the *Journal of Animal Ecology*.
...
Here, we present an accessible guide to modelling habitat selection from tracking data. We purposefully focus on building conceptual understanding rather than formal mathematical detail, with a particular emphasis on reconciling how different models define and approximate availability – a pervasive source of confusion for quantitative ecologists. In doing so, we clarify how all existing approaches can be understood through the lens of point process theory and articulate connections that are often difficult to discern from the literature.
...
With this paper, we aim to help readers make sense of the ecological and statistical literature and provide a practical reference to analysing tracking data for habitat selection studies.
